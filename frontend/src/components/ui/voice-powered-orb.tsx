...
interface VoicePoweredOrbProps {
  className?: string;
  state: OrbState;
  getLevel: () => number; // 0–1, polled each frame (mic level or AI output level)
}

// Per-state visual tuning: hue (color), base rotation speed, how strongly level drives motion.
const STATE_CFG: Record<OrbState, { hue: number; baseRot: number; reactivity: number }> = {
  idle:      { hue: 220, baseRot: 0.05, reactivity: 0.0 },
  listening: { hue: 200, baseRot: 0.15, reactivity: 1.0 },
  thinking:  { hue: 280, baseRot: 0.8,  reactivity: 0.0 },
  speaking:  { hue: 30,  baseRot: 0.25, reactivity: 1.0 },
};

export const VoicePoweredOrb: FC<VoicePoweredOrbProps> = ({ className, state, getLevel }) => {
  const ctnDom = useRef<HTMLDivElement>(null);
  // Live refs so the RAF loop reads current values without re-running the effect.
  ...
  useEffect(() => {
    ...
    try {
      ...
      let lastTime = 0;
      let currentRot = 0;
      let smoothHue = STATE_CFG[stateRef.current].hue;
      let smoothLevel = 0;
      ...
      const update = (t: number) => {
        ...
        smoothLevel += (level - smoothLevel) * 0.2;          // smooth jitter
        smoothHue += (cfg.hue - smoothHue) * 0.05;           // glide between state colors

        const reactive = smoothLevel * cfg.reactivity;
        currentRot += dt * (cfg.baseRot + reactive * 2.0);

        program.uniforms.iTime.value = t * 0.001;
        program.uniforms.hue.value = smoothHue;
        program.uniforms.rot.value = currentRot;
        program.uniforms.hover.value = Math.min(reactive * 2.0, 1.0);
        program.uniforms.hoverIntensity.value = Math.min(reactive * 0.8, 0.8);
